...
User.Id_Usuarios, User.Nombre, User.ApellidoP, User.ApellidoM, Of.Id_Oficio, Of.NombreOf, Bank.Id_Bancos, Bank.NombreBanco, Pago.Id_TPago, Pago.DescripcionPago FROM Expediente Ex INNER JOIN Usuarios User ON Ex.Id_UserExp = User.Id_Usuarios 
INNER JOIN Oficios Of ON Ex.Id_OficioUser = Of.Id_Oficio INNER JOIN Bancos Bank ON  Ex.Id_Banco = Bank.Id_Bancos INNER JOIN TPago Pago ON Ex.Id_FPago = Pago.Id_TPago WHERE Id_UserExp = '$IdUserExpediente'";
